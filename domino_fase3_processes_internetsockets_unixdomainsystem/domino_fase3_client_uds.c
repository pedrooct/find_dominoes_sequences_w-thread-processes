...
int main(int argc, char *argv[])
{
    ...
    int i=0,j=0,c, s, uds ,bytes;
    char buf[PIPE_BUF];
    struct sockaddr_un channel;
    int handsize= atoi(argv[1]); // guarda a conversão do string para inteiro no variavel , sendo o argv[1] o valor do tamanho da mão
    int seqnum=atoi(argv[2]); // guarda a conversão do string para inteiro no variavel , sendo o argv[2] o valor da peça inicial de jogo
    char *handnum=argv[3]; // guarda os valores da mão que vem por argv[3]
    int seqarray[2];
    int handarray[48];
    char p[1];
    char *timebuf;
    int time;
    time = open("/media/sf_SO_dropbox/newteste/data/time_uds.txt", O_WRONLY| O_CREAT | O_TRUNC | O_APPEND, 0666); // abre ou cria ficheiro para guardar tempos
    if(time==-1)
    {
    ...
    for(i=0; i< strlen(argv[2]); i++) // converte as peças em valores e guarda indivualmmente numa posição do array a peça de jogo
    {
        p[0]=argv[2][i];
        seqarray[i]=atoi(p);
    }
    for(i=0; i<strlen(argv[3]); i++) // converte as peças da mão em valores e guarda indivualmmente numa posição do array
    {
        p[0]=argv[3][i];
        handarray[i]=atoi(p);
    }

    hand = read_hand_from_array(hand,handarray,handsize); // lê a mão do array

    BLOCK * peca = (BLOCK *)malloc(sizeof(BLOCK));
    peca=hand->pfirst;
    peca=associate(peca,seqarray); // associa uma peça a peça real , vai á procura da peça com os mesmos valores e igual-os
    ...
    if ( (uds = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) // criamos o socket
    {
        perror("socket error");
        exit(1);
    }
    ...
    if (connect(uds, (struct sockaddr*)&channel, sizeof(channel)) == -1) // abrimos a conexão
    {
        perror("connect error");
        exit(1);
    }
    save_file_sequences_write(all_sequences,uds); //escrevemos para o socket
...
void save_file_sequences_write(ALLSEQUENCES * pall_sequences, int uds)
{
    int bytes;
    ...
    while(paux!=NULL)
    {
        ...
        write(uds,readBuffer,strlen(readBuffer));
        paux=paux->pnext;
    }
}
/**
...
HAND *read_hand_from_file(HAND *phand, char *pathFile)
{
    ...
    if (fp != NULL)
    {
        ...
        {
            //fscanf(fp,"%d%d", &leftSide, &rightSide);
            phand->pfirst = insert(phand->pfirst, leftSide, rightSide, 1);
        }
    }
    fclose(fp);
