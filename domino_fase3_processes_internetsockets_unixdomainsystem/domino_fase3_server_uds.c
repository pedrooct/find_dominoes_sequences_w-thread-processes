...
char *socket_path = "/tmp/socket"; // caminho para o ficheiro temporario
char * path_client = "/media/sf_SO_dropbox/newteste/domino_fase3_client_uds.out"; // caminho para o .out do cliente
struct timeval end_time, end_write; // variaveis globais para guardar o tempo
...
int main(int argc, const char * argv[])
{
    ...
    int listenfd,connfd,fd,bytes; // variaveis especificas
    char buff[PIPE_BUF];
    char  buffhand[1024];

    struct sockaddr_un channel_srv; // estrutura para criar canal do socket
    ...
    if ( (listenfd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {  // criar socket do servidor
        perror("socket error");
        exit(-1);
    }
    ...
    memset(&channel_srv, 0, sizeof(channel_srv)); // colocar todos os valores da estrutura a 0
    channel_srv.sun_family = AF_UNIX;
    strncpy(channel_srv.sun_path, socket_path, sizeof(channel_srv.sun_path)-1);

    if (bind(listenfd, (struct sockaddr*)&channel_srv, sizeof(channel_srv)) == -1) {      // Interligar o socket do servidor ao nome do mesmo
        perror("bind error");
        exit(-1);
    }
    if (listen(listenfd, hand->hand_size) == -1) {   // Configurar a queue , que é o tamanho da mão
        perror("listen error");
        exit(-1);
    }
    ...
    BLOCK * pecaaux=NULL;
    pecaaux=hand->pfirst;
    while(pecaaux!=NULL) // converter os valores das peças em uma string contendo todos os valores
    {
        sprintf(buffhand,"%s%d%d",buffhand,pecaaux->left_side,pecaaux->right_side);
        pecaaux=pecaaux->pnext;
    }
    char handsize[255];
    sprintf(handsize,"%d",hand->hand_size); // tamanho da mão em string
    ...
    for(i=0; i<hand->hand_size; i++)
    {
        ...
        {
            char buffseq[1024];
            sprintf(buffseq,"%d%d",seq->pfirst->left_side,seq->pfirst->right_side); // convertemos a peça inicial de jogo em string e criamos o filho
            printf("created  \n");
            execl(path_client,"domino_fase3_client_uds.out",handsize,buffseq,buffhand,(char *)0); // executamos o cliente e enviamos por parametro o tamanho da mão, a peça inicial de jogo e a mão
            perror("execl");
            exit(1);
    ...
    for(i=0; i<hand->hand_size; i++)
    {
         if ((connfd = accept(listenfd, NULL, NULL)) == -1) { // se a conexão for aceite podemos começar a escrever , senão ele salta uma iteração
            perror("accept error");
            continue;
        }
        save_file_sequences_write(all_sequences,save,connfd);
        kill(getpid(),SIGUSR2);
        close(connfd);// fecha a conexão ao fim da escrita
    }
    ...
    printf("time of pipe comunication = %f seconds\n",(double) (end_write.tv_usec - end_time.tv_usec) / 1000000 +(double) (end_write.tv_sec - end_time.tv_sec));
