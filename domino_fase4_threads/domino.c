...
char *path_file_hand = "/media/sf_newteste/threads_2/data/mao20P.txt";

//Mão global do cador
HAND *handG; // mão global
ALLSEQUENCES *allseq; //variavel allsequences global

pthread_t thread_ids[28]; // armazena os ids das threads

BLOCK * associate(BLOCK *pfirst,int pos) // procura a peça para iniciar a sequencia
{
    int i=0;
    while(pfirst!=NULL)
    {
...
void *threadhandler(void * pos) // código a ser executado pela thread
{
    int i;
    i=(int) pos; // conversão do valor de I que é enviado por parametro
    HAND * handst; // mão local
    handst = (HAND *) malloc(sizeof(HAND));
    handst->pfirst=NULL;
    handst->hand_size=0;

    //handst=handG;
    handst=copyHand(handG,handst,handG->hand_size); // vai fazer uma cópia individual da mão para cada thread
    BLOCK * peca = (BLOCK *)malloc(sizeof(BLOCK));
    peca=handst->pfirst;
    peca=associate(peca,i); // peça para iniciar a busca
    ...
    peca->available=0;
    recursive_backtrack(handst,seq,allseq+i, 1);
...
int main(int argc, const char * argv[])
{
    ...
    struct timeval tfather,tfather_end, tthread_time, tthread_end;
    ...
    allseq = (ALLSEQUENCES *) malloc(sizeof(ALLSEQUENCES)* handG->hand_size);
    for(i=0; i<handG->hand_size; i++)
    {
        allseq[i].pfirst = NULL;
        allseq[i].path_file = (char *) malloc(sizeof(char)*150);
        strcpy(allseq[i].path_file, path_save_file);
        allseq[i].number_of_sequences = 0;
    }

    gettimeofday(&tthread_time, NULL);
    for(i=0; i<handG->hand_size; i++) // cria as threads
    {
        pthread_create(&thread_ids[i],NULL,&threadhandler,(void*) i);
        printf("Criada %d thread %lu \n",i,(unsigned long)thread_ids[i]);
    }
    for(i=0; i<handG->hand_size; i++) // espera que todas as sequencias acabem
    {
        pthread_join(thread_ids[i],NULL);
        printf("thread %d finish\n",i);
    }
    gettimeofday(&tthread_end, NULL);
    for(i=0; i<handG->hand_size; i++) // vamos buscar as posições na estrutura e escrever os seus dados para 1 ficheiro
    {
        save_file_sequences_write(allseq+i,save,thread_ids[i]); //&return_thread
    }
...
void save_file_sequences_write(ALLSEQUENCES * pall_sequences, int save,long tids)
{
    int bytes;
    ...
    int n,file;
    while(paux!=NULL) // && paux->size_of_sequence<nsize
    {
        sprintf(buffer,"%lu-%s\n",(unsigned long)tids,paux->sequence); // ecreve o thread id
...
HAND *copyHand(HAND * oHand,HAND * dHand, int nsize) // copia a mão da estrutura para a thread local
{
    short num_blocks = 0, leftSide = 0, rightSide = 0;
    HAND *pprim = dHand;
    num_blocks=nsize;
